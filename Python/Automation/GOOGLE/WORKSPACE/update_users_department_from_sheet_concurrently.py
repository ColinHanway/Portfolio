...
def update_user(email, department):
    try:
        updated_user = gw.update_user_department(email, department)
        return f"Updated {email} with department {department}"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in tqdm(
            concurrent.futures.as_completed(
                {executor.submit(update_user, email, department): (email, department) for email, department in sheet_data}
            ),
