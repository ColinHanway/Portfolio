...
import requests
import pandas as pd
from Utils import NinjaOne_Keys

STRING_ACCESS_KEY_ID = NinjaOne_Keys.STRING_ACCESS_KEY_ID
STRING_SECRET_ACCESS_KEY = NinjaOne_Keys.STRING_SECRET_ACCESS_KEY
...
def request(use_path):
    ...
    hmac_key = bytes(STRING_SECRET_ACCESS_KEY, "utf-8")
    ...
    auth_header = f"NJ {STRING_ACCESS_KEY_ID}:{signature_base64}"
    ...
    def __init__(self, org_dict):
        ...
        for self.location in self.org_locations:
            for self.item in self.location:
                self.item["installer"] = request(f"/v2/organization/{self.org_id}/location/{self.item["id"]}/installer/WINDOWS_MSI") # return f"/v2/organization/{self.org_name}/location/{location["id"]}/installer/WINDOWS_MSI"
    ...
    for object in request(ORGANIZATIONS_PATH):
        ...
        for location in org.get_org_locations():
            ...
            for item in location:
                print(item["name"], item["installer"]['url'])
                output_dict["Organization"] = org_name
                output_dict["Location"] = item["name"]
                output_dict["Installer"] = item["installer"]['url']
                df = pd.DataFrame(output_dict, index=[i])
                output_df = pd.concat([output_df, df])
                i += 1
            print("")

    with pd.ExcelWriter("G:/Shared drives/Corporate IT/NinjaOne/links.xlsx") as writer:
        output_df.sort_values("Organization").to_excel(writer)
